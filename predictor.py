...
class CreditRiskPredictor:
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            # This ensures paths work regardless of where the API is started from
            script_dir = os.path.dirname(os.path.abspath(__file__))
            project_root = script_dir  # predictor.py is in the project root
            
            # Prefer loading the latest versioned artifacts from models/manifest.json if present
            manifest_path = os.path.join(project_root, "models", "manifest.json")
            ...
            if self.model is None:
                model_path = os.path.normpath(os.path.join(project_root, MODEL_PKL))
                logger.info(f"Loading model from fallback path: {model_path}")
                if not os.path.exists(model_path):
                    raise FileNotFoundError(f"Model file not found at: {model_path}")
                self.model = joblib.load(model_path)
            if self.scaler is None:
                scaler_path = os.path.normpath(os.path.join(project_root, SCALER_PKL))
                logger.info(f"Loading scaler from fallback path: {scaler_path}")
                if not os.path.exists(scaler_path):
                    raise FileNotFoundError(f"Scaler file not found at: {scaler_path}")
                self.scaler = joblib.load(scaler_path)
            if self.feature_names is None:
                features_path = os.path.normpath(os.path.join(project_root, FEATURE_NAMES_PKL))
