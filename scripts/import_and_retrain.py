...
def show_summary(db):
    ...
    try:
        ...
        with_feedback = db.query(Prediction)\
            .filter(Prediction.actual_outcome.isnot(None))\
            .count()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import CSV dataset and retrain model"
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--skip-import",
        action="store_true",
        help="Skip import, only retrain with existing data"
    )
    parser.add_argument(
        "--skip-retrain",
        action="store_true",
        help="Only import data, skip retraining"
    )
    parser.add_argument(
        "--clear-first",
        action="store_true",
        help="Clear existing predictions before import"
    )
